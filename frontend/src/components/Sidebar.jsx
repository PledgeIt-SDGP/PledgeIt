...
const MenuItem = ({ icon, name, path, isActive }) => {
  return (
    ...
    `}>  
      {React.cloneElement(icon, { "aria-hidden": true, className: "h-5 w-5 ml-3 " })}
      <Link
        to={path}
...
const Sidebar = () => {
  ...
  return (
    <div className="flex flex-col h-screen md:w-64 sm:w-0 bg-white shadow-md border-r border-gray-100">
      ...
      {/* Sidebar Content */}
      <div className={`flex flex-col h-screen w-64 bg-gray-50 shadow-md border-r border-gray-100 ${isOpen ? "block" : "hidden sm:block md:block"}`}>
